...
const keywordResultSchema = new Schema({
	...
	center_nm: String,
	entity_info: [
		{ entity_cd: String, freq: Number }
	],
	keyword_num : [
		{ unique_keyword_len: Number, tot_keyword_len: Number }
	],
	keywordInfo : [
		{ keyword: String, keyword_offset: String, freq: Number }
	]
...
keywordResultSchema.pre('find', function() {
	...
	console.log('[tbl_keyword_result] 쿼리시작시간 (find) : ' + currentTime);
...
keywordResultSchema.post('find', function(result) {
	...
	const utility = require('../commons/utility');
	const currentTime = utility.getCurrentTime('YYYY-MM-DD HH:mm:ss');
	console.log('[tbl_keyword_result] 쿼리종료시간 (find) : ' + currentTime);

	const operTime = (Date.now() - this.start) / 1000; // sec
	console.log('[tbl_keyword_result] 쿼리소요시간 (find) : ' + operTime.toFixed(2) + ' sec');
});


// Create Model & Export
module.exports = mongoose.model('tbl_keyword_result', keywordResultSchema); //collection name : tbl_keyword_result